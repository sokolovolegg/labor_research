"""
Pracuj.pl — Wayback Machine Historical Job Scraper
===================================================
Poland (EU East), period 2016–2024.

URL Patterns for pracuj.pl jobs:
  /oferta-pracy/<slug>,<job_id>   — main format
  /praca/<slug>,<job_id>          — old format

Usage:
  python scraper_pracuj_pl.py
  python scraper_pracuj_pl.py --dedup
"""
...
OUTPUT_CSV = os.path.join(ROOT_DIR, "data", "raw", "pracuj_pl_jobs.csv")
LOG_FILE   = os.path.join(ROOT_DIR, "logs", "pracuj_pl.log")
...
JUNK_MARKERS = [
    ...
    "this snapshot was not found", "nie znaleziono strony", "strona nie istnieje",
]
...
def fetch_cdx_urls() -> list[dict]:
    ...
    for pattern in CDX_URL_PATTERNS:
        params = (
            f"url={pattern}&output=json&collapse=urlkey"
            f"&from={CDX_FROM}&to={CDX_TO}"
            f"&fl=timestamp,original&filter=statuscode:200&limit=100000"
        )
        full_url = f"{CDX_URL}?{params}"
        log.info(f"CDX request: {full_url}")
        try:
            with urllib.request.urlopen(full_url, timeout=90) as resp:
                raw = json.loads(resp.read().decode("utf-8"))
        except Exception as e:
            log.error(f"CDX error for {pattern}: {e}")
            continue

        if not raw or len(raw) < 2:
            log.warning(f"CDX empty result: {pattern}")
            continue

        rows = raw[1:]
        log.info(f"  Received {len(rows)} records for {pattern}")
    ...
    log.info(f"Total unique vacancies: {len(result)}")
    return result
...
def _is_job_url(url: str) -> bool:
    url_lower = url.lower()
    exclude = [
        "/oferta-pracy?", "/praca?", "/praca/?",
        "/firma/", "/porady/", "/kandydat/",
        "/szukaj/", "/wyniki/", "pracuj.pl/$",
        "pracuj.pl/index", "/logowanie", "/rejestracja",
    ]
    for ex in exclude:
        if ex in url_lower:
            return False
    include = ["/oferta-pracy/", "/praca/"]
    # URL must contain numeric ID
    has_id = bool(re.search(r",\d{4,}", url_lower))
    return any(inc in url_lower for inc in include) and has_id
...
#  PARSING
# ─────────────────────────────────────────────
def parse_pracuj_html(html: str, original_url: str, timestamp: str) -> dict | None:
    ...
    for tag, attrs in [
        ...
        ("h1", {"class": re.compile(r"offer-title|jobTitle|position", re.I)}),
        ("h1", {}),
    ]:
    ...
    for tag, attrs in [
        ...
        ("a",  {"data-test": "anchor-company-link"}),
        ("span", {"class": re.compile(r"company|employer|pracodawca", re.I)}),
        ("h2", {"class": re.compile(r"company|employer", re.I)}),
    ]:
        el = soup.find(tag, attrs)
        if el and el.get_text(strip=True):
            job["company"] = _clean(el.get_text(strip=True))
            break

    for tag, attrs in [
        ("li",   {"data-test": "sections-benefit-workplaces"}),
        ("span", {"data-test": "text-workplaceAddress"}),
        ("span", {"class": re.compile(r"location|city|miejsce|lokalizacja", re.I)}),
    ]:
        el = soup.find(tag, attrs)
        if el and el.get_text(strip=True):
            job["city"] = _clean(el.get_text(strip=True).split(",")[0])
            break

    for tag, attrs in [
        ("span", {"data-test": "text-addDate"}),
        ("time", {}),
    ...
    for tag, attrs in [
        ...
        ("span", {"class": re.compile(r"employment|etat|zatrudnienie", re.I)}),
    ]:
        el = soup.find(tag, attrs)
        if el and el.get_text(strip=True):
            job["employment_type"] = _clean(el.get_text(strip=True))
            break

    for tag, attrs in [
        ("div",     {"data-test": "section-description"}),
        ("section", {"data-test": "section-requirements"}),
        ("div",     {"class": re.compile(r"offer-description|job-description|opis", re.I)}),
        ("div",     {"id": re.compile(r"description|offerDescription", re.I)}),
    ]:
        el = soup.find(tag, attrs)
        if el:
...
async def fetch_and_parse(session, semaphore, entry, csv_lock, csv_path, stats):
    ...
    async with semaphore:
        ...
        job = parse_pracuj_html(html, original_url, timestamp)
        if job is None:
            stats["empty"] += 1
            return
...
async def main():
    log.info("=" * 60)
    log.info("Pracuj.pl — Wayback Machine Scraper")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        try:
            await tqdm.gather(*tasks, desc="Scraping Pracuj.pl", unit="page")
