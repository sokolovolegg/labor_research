"""
eFinancialCareers.com — Wayback Machine Historical Job Scraper
==============================================================
Global Finance/Banking portal, period 2016–2024.
Covers: USA, UK, Europe, Asia.

URL Patterns:
  /jobs-<slug>.id<job_id>          — main format
  /jobs-<slug>.br<job_id>          — alternative
  /jobs-<slug>.ry<job_id>          — another variant
  /job/preview/<job_id>            — preview format

Usage:
  python scraper_efinancialcareers.py
  python scraper_efinancialcareers.py --dedup
"""
...
OUTPUT_CSV = os.path.join(ROOT_DIR, "data", "raw", "efinancialcareers_jobs.csv")
LOG_FILE   = os.path.join(ROOT_DIR, "logs", "efinancialcareers.log")
...
CDX_URL_PATTERNS = [
    "efinancialcareers.com/jobs-*",
    "efinancialcareers.com/job/preview/*",
]
CDX_FROM        = "20160101"
CDX_TO          = "20241231"
...
def _is_job_url(url: str) -> bool:
    url_lower = url.lower()
    exclude = [
        "/jobs/", "/jobs?", "efinancialcareers.com/$",
        "/search/", "/news/", "/advice/",
        "/employer/", "/recruiter/",
        "?page=", "?q=",
    ]
    for ex in exclude:
        if ex in url_lower:
            return False
    # Main format
    has_job_id = bool(re.search(r"\.(id|br|ry)\d+", url_lower))
    # Preview format
    has_preview = bool(re.search(r"/job/preview/\d+", url_lower))
    return has_job_id or has_preview
...
def extract_job_id(url: str) -> str:
    m = re.search(r"\.(id|br|ry)(\d+)", url, re.I)
    if m:
        return m.group(2)
    m = re.search(r"/job/preview/(\d+)", url)
    if m:
        return m.group(1)
...
#  PARSING
# ─────────────────────────────────────────────
def parse_efc_html(html: str, original_url: str, timestamp: str) -> dict | None:
    soup = BeautifulSoup(html, "lxml")

    country = _extract_country(original_url)
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        ...
        if isinstance(addr, dict):
            ...
            if not country:
                job["country"] = _clean(addr.get("addressCountry", ""))
    ...
    for tag, attrs in [
        ("h1", {"class": re.compile(r"job-title|jobTitle|position-title", re.I)}),
        ("h1", {"itemprop": "title"}),
    ...
    for tag, attrs in [
        ("span", {"class": re.compile(r"company|employer|firm", re.I)}),
    ...
    for tag, attrs in [
        ...
        ("span", {"class": re.compile(r"location|city|place", re.I)}),
        ("li",   {"class": re.compile(r"location|city", re.I)}),
    ...
    for tag, attrs in [
        ...
        ("time", {}),
        ("span", {"class": re.compile(r"date|posted|published", re.I)}),
    ...
    for tag, attrs in [
        ...
        ("li",   {"class": re.compile(r"employment|contract|permanent", re.I)}),
    ]:
        el = soup.find(tag, attrs)
        if el:
            text = el.get("content") or el.get_text(strip=True)
    ...
    for tag, attrs in [
        ...
        ("div",     {"class": re.compile(r"job-description|jobDescription|job-detail|description", re.I)}),
        ("section", {"class": re.compile(r"description|job-detail", re.I)}),
...
def _extract_country(url: str) -> str:
    country_map = {
...
def init_csv(csv_path: str):
    if not os.path.exists(csv_path):
        ...
        log.info(f"Resuming writing: {csv_path}")
...
async def fetch_and_parse(session, semaphore, entry, csv_lock, csv_path, stats):
    ...
    async with semaphore:
        ...
        job = parse_efc_html(html, original_url, timestamp)
        if job is None:
            stats["empty"] += 1
            return
        ...
        log.info(
            ...
            f"{job['company'][:30]!r} | {job['city']} | {job['country']} | {job['date_posted']}"
        )
...
async def main():
    log.info("=" * 60)
    log.info("eFinancialCareers.com — Wayback Machine Scraper")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        try:
            await tqdm.gather(*tasks, desc="Scraping eFinancialCareers", unit="page")
...
def deduplicate():
    ...
    with open(out, "w", newline="", encoding="utf-8") as f:
        ...
        for row in sorted(rows.values(), key=lambda r: r.get("timestamp", "")):
            w.writerow(row)
    print(f"Deduplication: {len(rows)} records → {out}")
