"""
Stack Overflow Jobs — Wayback Machine Historical Job Scraper
============================================================
Global IT job board, 2016–2024.
Focus: Software Engineering, DevOps, Data Science, IT.

URL Pattern:
  /jobs/<id>/<slug>   — main format

Usage:
  python scraper_stackoverflow_jobs.py
  python scraper_stackoverflow_jobs.py --dedup
"""
...
OUTPUT_CSV = os.path.join(ROOT_DIR, "data", "raw", "stackoverflow_jobs.csv")
LOG_FILE   = os.path.join(ROOT_DIR, "logs", "stackoverflow_jobs.log")
...
JUNK_MARKERS = [
    ...
    "ddos-guard", "just a moment", "wayback machine doesn't have",
    "got an http 429 error", "this snapshot was not found",
    "job not found", "this job is no longer available",
    "this position has been filled", "page not found",
    "no longer accepting", "job has expired",
]
...
def _is_job_url(url: str) -> bool:
    ...
    exclude = [
        "/jobs/feed", "/jobs/search", "/jobs?", "/jobs/directory",
        "/jobs/companies", "/jobs/developer-jobs-using-",
        "so_medium=", "so_source=",  
        "/apply", "/save", "/unsave",
    ]
    for ex in exclude:
        if ex in url_lower:
            return False
    # Valid job: /jobs/<numeric id>/<slug>
    return bool(re.search(r"/jobs/\d+/[a-z0-9-]+", url_lower))
...
#  PARSING
# ─────────────────────────────────────────────
def parse_so_html(html: str, original_url: str, timestamp: str) -> dict | None:
    ...
    for tag, attrs in [
        ("h1", {"class": re.compile(r"job-detail.*title|fs-headline", re.I)}),
        ("h1", {"itemprop": "title"}),
        ("a",  {"class": re.compile(r"job-link|position", re.I)}),
        ("h1", {}),
    ]:
    ...
    for tag, attrs in [
        ("div", {"class": re.compile(r"employer|company", re.I)}),
        ("span", {"class": re.compile(r"employer|company", re.I)}),
        ("a",   {"class": re.compile(r"employer|company", re.I)}),
        ("span", {"itemprop": "name"}),
    ]:
        el = soup.find(tag, attrs)
        if el and el.get_text(strip=True):
    ...
    for tag, attrs in [
        ("span", {"class": re.compile(r"location|geoTag", re.I)}),
    ...
    for tag, attrs in [
        ("span", {"class": re.compile(r"employment|job.type|contract|fulltime|remote", re.I)}),
    ...
    for tag, attrs in [
        ...
        ("div", {"class": re.compile(r"job-description|description|detail", re.I)}),
...
async def fetch_and_parse(session, semaphore, entry, csv_lock, csv_path, stats):
    ...
    async with semaphore:
        ...
        job = parse_so_html(html, original_url, timestamp)
        if job is None:
            stats["empty"] += 1
            return
...
async def main():
    log.info("=" * 60)
    log.info("Stack Overflow Jobs — Wayback Machine Scraper")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        try:
            await tqdm.gather(*tasks, desc="Scraping SO Jobs", unit="page")
