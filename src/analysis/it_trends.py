...
FILE_PATH = os.path.join(BASE_DIR, "data", "processed", "IT_LIGHTCAST_SKILLS.csv")
...
def process_trends(df, skill_column, prefix):
    ...
    global_path = os.path.join(BASE_DIR, "data", "processed", f"IT_TOP_30_GLOBAL_{prefix}.csv")
    country_path = os.path.join(BASE_DIR, "data", "processed", f"IT_TOP_30_BY_COUNTRY_{prefix}.csv")
    
    top_30_global.to_csv(global_path, index=False)
    top_30_by_country.to_csv(country_path, index=False)

if __name__ == "__main__":
    columns_to_keep = ['year_month', 'country', 'hard_skills', 'soft_skills']
